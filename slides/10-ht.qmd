...
- The significant level $\alpha$ determines [*how rare or unlikely our evidence must be*]{.blue} in order to represent sufficient evidence against $H_0$.

. . .

- $\alpha$ is the [maximum chance you are willing to take of a false alarm]{.blue}, saying *there is an effect when in reality there is no effect*: $\alpha = P(\text{Reject } H_0 \mid H_0 \text{ is true})$
...
plot(x, hx, type="l", lty = 1, xlab = "", lwd = 4, las = 1, cex.main = 1.5,
...
```{r}
knitr::include_graphics("./images/10-ht/test-type.png")
```
...
```{r}
knitr::include_graphics("./images/10-ht/rejection.png")
```
...
| **$p$-value under $H_0$**  | $H_1: \mu < \mu_0$ <br> $P(Z \le z_{test} \mid H_0)$ | $H_1: \mu < \mu_0$ <br> $P(T_{n-1} \le t_{test} \mid H_0)$ |
|    | $H_1: \mu > \mu_0$ <br> $P(Z \ge z_{test} \mid H_0)$ | $H_1: \mu > \mu_0$ <br> $P(T_{n-1} \ge t_{test} \mid H_0)$ |
|   | $H_1: \mu \ne \mu_0$ <br> $2P(Z \ge \, |z_{test}| \mid H_0)$ | $H_1: \mu \ne \mu_0$ <br> $2P(T_{n-1} \ge  \, |t_{test}| \mid H_0)$ |
